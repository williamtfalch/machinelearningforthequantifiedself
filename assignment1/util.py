...
def print_latex_table_statistics_two_datasets(dataset1, dataset2):
    ...
    for col in dataset1.columns:
        print(col, '&')
        print_table_cell((float((dataset1_length - dataset1[col].count()))/dataset1_length)*100, (float(
            (dataset2_length - dataset2[col].count()))/dataset2_length)*100)
        print(' & ')
...
def print_latex_statistics_clusters(dataset, cluster_col, input_cols, label_col):
    label_cols = [c for c in dataset.columns if label_col ==
                  c[0:len(label_col)]]
    ...
    for c in input_cols:
        ...
        for cluster in clusters:
            print(' & ', "{0:.2f}".format(
                dataset.ix[dataset[cluster_col] == cluster, c].mean()))
        print('\\\\')
        print(' & std ')
        for cluster in clusters:
            print(' & ', "{0:.2f}".format(
                dataset.ix[dataset[cluster_col] == cluster, c].std()))
        print('\\\\')

    for l in label_cols:
        print(l, ' & percentage ')
        for cluster in clusters:
            print(' & ', "{0:.2f}".format((float(dataset.ix[dataset[cluster_col] == cluster, l].sum(
            ))/len(dataset[dataset[l] == 1].index) * 100)), '\%')
        print('\\\\')
...
def print_table_row_performances(row_name, training_len, test_len, values):
    ...
    for val in values:
        ...
        print('\\emph{(', "{0:.4f}".format(
            val[0]-2*sd_train), '-', "{0:.4f}".format(val[0]+2*sd_train), ')}', ' & ')
        sd_test = math.sqrt((val[1]*(1-val[1]))/test_len)
        print("{0:.4f}".format(val[1]))
        print('\\emph{(', "{0:.4f}".format(
            val[1]-2*sd_test), '-', "{0:.4f}".format(val[1]+2*sd_test), ')}')
...
def print_pearson_correlations(correlations):
    for i in range(0, len(correlations)):
        if np.isfinite(correlations[i][1]):
            print(correlations[i][0], ' & ', "{0:.4f}".format(
                correlations[i][1]), '\\\\\\hline')
