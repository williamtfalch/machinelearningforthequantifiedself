...
class FourierTransformation:
    ...
    def abstract_frequency(self, data_table, cols, window_size, sampling_rate):
        ...
        for col in cols:
            ...
            for freq in freqs:
                data_table[col + '_freq_' +
                           str(freq) + '_Hz_ws_' + str(window_size)] = np.nan
            ...
            for col in cols:
                ...
                for j in range(0, len(freqs)):
                    data_table.ix[i, col + '_freq_' +
                                  str(freqs[j]) + '_Hz_ws_' + str(window_size)] = real_ampl[j]
                # And select the dominant frequency. We only consider the positive frequencies for now.

                data_table.ix[i, col +
                              '_max_freq'] = freqs[np.argmax(real_ampl[0:len(real_ampl)])]
                data_table.ix[i, col + '_freq_weighted'] = float(
                    np.sum(freqs * real_ampl)) / np.sum(real_ampl)
                PSD = np.divide(np.square(real_ampl), float(len(real_ampl)))
                PSD_pdf = np.divide(PSD, np.sum(PSD))
                data_table.ix[i, col + '_pse'] = - \
                    np.sum(np.log(PSD_pdf) * PSD_pdf)
